...
from svc.seed_vc.hf_utils import load_custom_model_from_hf
from svc.seed_vc.modules.commons import *
...
def load_models(args):
    if not args.f0_condition:
        ...
        # f0 extractor
        from svc.seed_vc.modules.rmvpe import RMVPE
    ...
    from svc.seed_vc.modules.campplus.DTDNN import CAMPPlus
    ...
    from svc.seed_vc.modules.bigvgan import bigvgan
    ...
    if model_params.speech_tokenizer.type == "facodec":
        ckpt_path, config_path = load_custom_model_from_hf(
            "Plachta/FAcodec", "pytorch_model.bin", "config.yml"
        )
    ...
    mel_fn_args = {
        ...
        "fmax": (
            None if config["preprocess_params"].get("fmax", "None") == "None" else 8000
        ),
        "center": False,
    }
    from svc.seed_vc.modules.audio import mel_spectrogram
...
def main(args):
    ...
    prompt_condition, _, prompt_codes, commitment_loss, codebook_loss = (
        model.length_regulator(S_ori, ylens=target2_lengths, n_quantizers=3, f0=F0_ori)
    )
